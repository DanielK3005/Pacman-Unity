...
public class GhostHome : GhostBehavior
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (enabled && collision.gameObject.layer == LayerMask.NameToLayer("Walls"))
        {
    ...
    private IEnumerator ExitTransition()
    {
        ...
        // Check if a Rigidbody2D component is attached before using it
        Rigidbody2D rb = ghost.movement.GetComponent<Rigidbody2D>();
        if (rb != null)
        {
            rb.isKinematic = true;
        }
